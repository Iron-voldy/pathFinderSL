...
import {
  faCamera, faPaperclip, faXmark, faLocationDot, faHotel,
  faMap, faStar,
  ...
  const fetchHotel = async () => {
    try {
      ...
      setFormData({
        ...
        star_classification:  h.star_classification || '3-star',
        hotel_classification: h.hotel_classification || 'Hotel',
        hotel_address:        h.hotel_address || '',
        hotel_image:          h.hotel_image || '',
        country:              h.country || 'Sri Lanka',
        city:                 h.city || '',
        micro_location:       h.micro_location || '',
        hotel_status:         h.hotel_status ? h.hotel_status.toLowerCase() : 'active',
        longitude:            h.longitude != null ? String(h.longitude) : '',
        latitude:             h.latitude != null ? String(h.latitude) : '',
        markup:               h.markup != null ? String(h.markup) : '',
        sub_description:      h.sub_description || '',
        provider:             h.provider || '',
        trip_advisor_link:    h.trip_advisor_link || '',
      });
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        if (formData.markup !== '') payload.markup = Number(formData.markup);
      }
      ...
      const detail = error.response?.data?.errors?.body;
      const msg = detail
        ? detail.map(err => err.message).join('\n')
        : (error.response?.data?.message || error.response?.data?.error || 'Failed to update hotel');
      alert(msg);
    } finally {
      setUpdating(false);
    }
        ...
        <div className="add-hotel-layout">
          ...
          <form onSubmit={handleSubmit} className="hotel-form" noValidate>
              ...
              <div className="form-group">
                <label htmlFor="hotel_description">
                  ...
                  {formData.hotel_description && (
                    <span className={descLen < 50 ? 'char-count char-count--warn' : 'char-count char-count--ok'}>
                      {descLen} / 50 min
                    </span>
                  )}
                </label>
            ...
            <div className="form-section">
              ...
              <div className="form-row">
                ...
                <div className="form-group">
                  ...
                  <input type="text" id="micro_location" name="micro_location" value={formData.micro_location} onChange={handleChange} placeholder="e.g. Kandy, Mirissa, Galle, Ella, Yala..." />
                </div>
                <div className="form-group">
            ...
            <div className="form-section">
                    ...
                    <div className="upload-zone__placeholder">
                      ...
                      <span className="upload-text">
                        {formData.hotel_image
                          ? 'Drop a new image to replace existing'
              ...
              <div className="form-row">
                <div className="form-group">
                  <label htmlFor="markup">Price Per Night (LKR)</label>
                  <input
                    type="number"
                    id="markup"
                    name="markup"
