...
export default function BudgetPlanDetail() {
  ...
  const openReportModal = () => {
    setReportRange({ start: plan?.start_date || '', end: plan?.end_date || '' });
  ...
  const handleExportCSV = async () => {
    setExporting(true);
    try {
      const resp = await budgetAPI.getReport(id, {
        startDate: reportRange.start || undefined,
        endDate:   reportRange.end   || undefined,
      });
      const url = window.URL.createObjectURL(new Blob([resp.data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', `budget_report_${plan.plan_name.replace(/[^a-z0-9]/gi, '_').toLowerCase()}.csv`);
  ...
  return (
    <>
          ...
          <div className="bpd-topbar__inner">
            ...
            <div className="bpd-topbar__title">
              ...
              <div className="bpd-topbar__right">
                <button className="bpd-report-btn" onClick={openReportModal}>
        ...
        <div className="bpd-body">
          ...
          <aside className="bpd-budget-sidebar">
            ...
            <div className="bpd-bs-card bpd-bs-info">
              ...
              <div className="bpd-bs-info__actions">
                <button className="bpd-bs-info__btn" onClick={() => openAddItem()}>+ Add Expense</button>
                <button className="bpd-bs-info__btn bpd-bs-info__btn--report" onClick={openReportModal}>
                  📥 Download Report
                </button>
              </div>
            </div>
      ...
      {/* Report Modal */}
      {showReportModal && (
        <div className="bp-modal-backdrop" onClick={() => setShowReportModal(false)}>
          <div className="bp-modal" onClick={(e) => e.stopPropagation()}>
            <div className="bp-modal__head">
              <h2>Download Budget Report</h2>
              <button className="bp-modal__close" onClick={() => setShowReportModal(false)}>✕</button>
            </div>
            <div className="bp-report-modal-body">
              <p style={{ marginBottom: '1.5rem', color: 'rgba(255,255,255,0.7)', fontSize: '0.9rem' }}>
                Select the date range for your report. Leave blank to download all days.
              </p>
              ...
              <div className="bp-form">
                ...
                <div className="bp-form__actions">
                  <button type="button" className="bp-btn-ghost" onClick={() => setShowReportModal(false)}>Cancel</button>
                  <button type="button" className="bp-btn-primary" onClick={handleExportCSV} disabled={exporting}>
