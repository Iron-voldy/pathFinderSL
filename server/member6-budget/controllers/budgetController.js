...
const exportBudgetReport = async (req, res, next) => {
  try {
    const { planId } = req.params;
    const { startDate, endDate } = req.query;

    const plan = await BudgetPlan.findOne({
      where: { id: planId, user_id: req.user.id },
      include: [{ model: BudgetItem, as: 'items' }],
    });

    if (!plan) return res.status(404).json({ success: false, message: 'Plan not found' });

    let filteredItems = (plan.items || []).map((i) => i.toJSON());

    if (startDate) {
      filteredItems = filteredItems.filter((i) => !i.item_date || i.item_date >= startDate);
    }
    if (endDate) {
      filteredItems = filteredItems.filter((i) => !i.item_date || i.item_date <= endDate);
    }

    // Sort by date then category
    filteredItems.sort((a, b) => {
      const dateA = a.item_date || '9999-12-31';
      const dateB = b.item_date || '9999-12-31';
      if (dateA !== dateB) return dateA.localeCompare(dateB);
      return (a.category || '').localeCompare(b.category || '');
    });

    const csvRows = [
      ['Date', 'Category', 'Item Name', 'Description', `Estimated Cost (${plan.currency})`, `Actual Cost (${plan.currency})`, 'Notes'],
    ];

    filteredItems.forEach((i) => {
      csvRows.push([
        i.item_date || 'Unscheduled',
        i.category,
    ...
    res.setHeader(
      'Content-Disposition',
      `attachment; filename=budget_report_${plan.id}.csv`
    );
